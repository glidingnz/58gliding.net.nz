	...
	<div>
		...
		<form v-if="status=='loaded'">
			<div class="form-group">
				<label for="">Send GNZ member approval notification email to:</label>
				<input type="text" class="form-control" id="email_new_member_to" v-model="email_new_member_to">
				(Comma separate for multiple. Usually the GNZ secratary)
			</div>
	...
	export default {
		...
		methods: {
			save: function() {
				...
				window.axios.post('/api/v1/settings', data).then(function (response) {
			...
			load: function() {
				var that = this;
				this.status='loading';
				window.axios.get('/api/v1/settings').then(function (response) {
