...
<template>
	...
	<h1 class="results-title"><a href="/aircraft">Aircraft</a> &raquo; <a href="/aircraft/{{aircraft.rego}}">{{aircraft.rego}}</a> &raquo; Edit</h1>
	...
	export default {
		...
		methods: {
			loadAircraft: function() {
				this.$http.get('/api/v1/aircraft/' + this.aircraftId, {params: this.state}).then(function (response) {
					var responseJson = response.json();

					this.aircraft = responseJson.data;
				});
			},
			save: function() {
				this.$http.put('/api/v1/aircraft/' + this.aircraft.rego, this.aircraft).then(function (response) {
